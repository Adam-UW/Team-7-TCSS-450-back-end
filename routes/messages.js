...
router.delete('/', (req, res, next)=>{
    console.log('delete route got hit')
    if(!req.body.memberid && !req.body.chatId){
        res.status(400).send({
            message: 'Missing required information'
        })
    }else if(isNaN(req.body.memberid)){
        response.status(400).send({
            message:"Malformed parameter. chatId must be a number"
        })
    }else{
    ...
    let values=[req.body.memberid]
    console.log('First block -> memberID ',values )
    ...
    let insert = `SELECT ChatId, Message, MemberId
                  FROM Messages
                  RETURNING PrimaryKey AS MessageId, ChatId, Message, MemberId AS email, TimeStamp`
    let values = [res.body.chatId, res.body.message, req.decoded.memberid]
    pool.query(insert)
        .then(result => {
            if (result.rowCount == 1) {
                //insertion success. Attach the message to the Response obj
    ...
    // delete a user 
    console.log('Inside last one => delete ')
    let deleteQUery=`DELETE FROM Chatmembers WHERE memberid=$1 and chatid=$2`
    let values= [req.body.memberid, req.body.chatId]

    pool.query(deleteQUery, values)
    .then(result=>{
       // console.log(`user with ${req.body.memberid} has been deleted from the chat`)
        if(result.rowCount==0){
            res.status(400).send({
                message: 'memberId is not deleted'
            })
        }else{
            // res.send({
            //     message: `user with memberId ${req.body.memberid} has been deleted`
            // })
            console.log(`User with memberId ${req.body.memberid} has been deleted`)
            next()
        }
    }).catch(err =>{
        res.status(400).send({
            message:"SQL ERROR before deletetion",
            err: err
        })
    })

}, (req, res)=>{
    console.log('At the pushy..')
    let temp = req.body.memberid
            // send a notification of this message to ALL members with registered tokens
            // let query = `SELECT token FROM Push_Token
            // INNER JOIN ChatMembers ON
            // Push_Token.memberid=ChatMembers.memberid
            // WHERE ChatMembers.chatId=$1`
            let query = `SELECT DISTINCT(token) FROM Push_Token WHERE Push_Token.memberid=$1; `
let values = [req.body.memberid]
console.log(values)
pool.query(query, values)
.then(result => {
...
// INSERT INTO ChatMembers VALUES(1, 65);
